					...
					go func(root string, age time.Duration, count int) {
						if paths, err := filepath.Glob(root); err == nil {
							...
							for _, path := range paths {
								...
								if err != nil || !file.WithinRoots(link, l.roots) {
									continue
								}
						...
						if paths, err := filepath.Glob(root); err == nil {
							...
							for _, path := range paths {
								...
								if info, err := os.Stat(path); err == nil && info.Mode().IsRegular() && time.Since(info.ModTime()) >= age {
									ok := false
									if source, err := os.OpenFile(path, os.O_RDONLY|file.O_NOFOLLOW, 0); err == nil {
...
func (l *ULog) Log(now time.Time, severity int, in any) {
	...
	if sfile {
		...
		if !strings.HasSuffix(path, file.Sep) && file.WithinRoots(path, l.roots) {
