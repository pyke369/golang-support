...
type Socket struct {
	Path, Agent, Remote, Protocol string
	Context                       any
	config                        *Config
	conn                          net.Conn
...
func Dial(endpoint, origin string, config *Config) (ws *Socket, err error) {
	...
	rproxy, _ := config.Proxy(url)

	request, err := http.NewRequest("GET", endpoint, http.NoBody)
	if err != nil {
		return nil, ustr.Wrap(err, "uws")
	}
	nonce, err := uhash.RandKey(16, "std")
	if err != nil {
		return nil, err
	}
	...
	if response, err := http.ReadResponse(bufio.NewReader(conn), request); err == nil {
		...
		ckey, path := sha1.Sum([]byte(nonce+UUID)), url.Path
		if path == "" {
			path = "/"
		}
		...
		if (protocol != "" && !slices.Contains(config.Protocols, protocol)) || (len(config.Protocols) > 0 && protocol == "" && config.NeedProtocol) {
			response.Body.Close()
			conn.Close()
...
func Handle(rw http.ResponseWriter, r *http.Request, config *Config) (handled bool, ws *Socket) {
	...
	if strings.Contains(strings.ToLower(r.Header.Get("Connection")), "upgrade") && strings.ToLower(r.Header.Get("Upgrade")) == "websocket" {
		...
		if dkey, err := base64.StdEncoding.DecodeString(ckey); err != nil || len(dkey) != 16 {
...
func (s *Socket) Write(mode byte, data []byte) (err error) {
	...
	length := len(data)
	if mode == OPCODE_TEXT || mode == OPCODE_BLOB {
		s.wlock.Lock()
		...
		readmore:
			for {
				if size < 0 {
					...
					bcontrol = opcode >= OPCODE_CLOSE && opcode <= OPCODE_PONG
					...
					case size == 126:
						...
						rsize := int(binary.BigEndian.Uint16(buffer[roffset+2:]))
						if rsize > s.config.MessageSize {
					...
					if (opcode > OPCODE_BLOB && size > 125) || (fin == 1 && size > s.config.MessageSize) {
						code = ERROR_OVERSIZED
						break close
					}
					if !bcontrol {
