...
func decode(in string, start, end time.Time) (out time.Time, err error) {
	if captures := matcher.FindStringSubmatch(strings.ToLower(in)); captures != nil {
		...
		if amount, err := strconv.Atoi(captures[3]); err == nil && amount > 0 {
			direction := +1
			if captures[2] == "-" {
				direction = -1
			}
...
func main() {
	...
	case "metadata":
		...
		metrics, err := mstore.New(os.Args[2])
		bail(err, 3)
		...
		for index, column := range columns {
			...
			if description := j.String(column["description"]); description != "" && !strings.HasSuffix(description, "...") {
				...
				for _, line := range data {
					if value := j.Slice(line); index+1 < len(value) {
						value := value[index+1]
	...
	case "import":
		...
		metrics, err := mstore.New(os.Args[2], true)
		bail(err, 5)
		metric, err := metrics.Metric(os.Args[3])
		bail(err, 6)
